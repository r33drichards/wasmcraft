...
-- wasm->Lua-source transpiler must agree. The transpiler emits Lua 5.1 source,
-- so this also passes on Cobalt with binary chunks disabled (the CC:T >= 1.109
-- situation the transpiler exists for).
package.path = "src/?.lua;test/?.lua;" .. package.path
local T = require("harness")
local wasm = require("wasm")
local decoder = require("decoder")
if _VERSION ~= "Lua 5.1" then
  print("0/0 assertions passed (transpiler test is Cobalt-only)")
  print("ALL_PASS")
  return
end
local transpiler = require("transpiler")
-- Never fall back to the interpreter: a transpile failure is a test failure.
transpiler._force_transpile = true
-- Route every function through the flattened-dispatcher path with
-- FORCE_OVERSIZED=1 to exercise transpile_oversized differentially.
transpiler._force_oversized = os.getenv("FORCE_OVERSIZED") == "1"
T.start("transpiler")
...
local function diff(name, calls)
  ...
  local ok, ci = pcall(transpiler.instantiate, decoder.load(b), {})
  if not ok then T.ok(false, name .. " transpiles: " .. tostring(ci)); return end
  for _, c in ipairs(calls) do
    local label = name .. ":" .. table.concat(c, ",")
    local r1 = { ii:call((table.unpack or unpack)(c)) }
    local ok2, r2 = pcall(function() return { ci:call((table.unpack or unpack)(c)) } end)
    if not ok2 then
      T.ok(false, label .. " runs: " .. tostring(r2))
    else
      local same = #r1 == #r2
      for k = 1, #r1 do if not approx(r1[k], r2[k]) then same = false end end
      T.ok(same, label .. "  interp=" .. tostring(r1[1]) .. " transpiled=" .. tostring(r2[1]))
    end
  end
end
