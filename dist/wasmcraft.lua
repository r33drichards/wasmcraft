...
local function cc_hostfs(root)
  ...
  return {
    read = function(p) p = full(p); if not fs.exists(p) or fs.isDir(p) then return nil end local h = fs.open(p, "rb"); if not h then return nil end local d = h.readAll(); h.close(); return d end,
    write = function(p, d) p = full(p); local h, e = fs.open(p, "wb"); if not h then error("hostfs write " .. p .. ": " .. tostring(e), 0) end h.write(d); h.close() end,
